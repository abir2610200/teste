...
import {  MonetizationOn, Delete, FilterList, ArrowDownward, ArrowUpward, Print, FileDownload } from "@mui/icons-material";
...
export default function RueList() {
  ...
  const fetchComptes = async () => {
    try {
      const response = await axios.get("http://localhost:5000/livraisons/api/livre");
              ...
              .map((compte) => (
                ...
                >
                  ...
                  <TableCell sx={{ width: { xs: "25%", sm: "20%" }, textAlign: "center" }}>
                    <IconButton color="success" onClick={() => handleValid(compte._id)}>
                      <MonetizationOn />
                    </IconButton>
